...
Cursor SDK variant display names are presentation metadata, not unique identifiers. A variant may describe a composite parameter tuple such as context, reasoning, and fast mode while sharing its display name with other variants. When Paseo projects one parameter dimension into model-picker rows, such as context, derive the row suffix from that parameter value's `displayName` unless a variant exactly matches the row's complete parameter set. Keep the encoded model identity as the SDK model id plus its selected params; do not infer identity from a variant display name.
