...
describe("CursorSdkAgentClient", () => {
  ...
  test("listModes marks YOLO unattended and includes Sandbox only when side-effect-free support passes", async () => {
    const runtime = new FakeCursorSdkRuntime();
    const client = createClient(runtime, {
      env: {
        CURSOR_API_KEY: "provider-secret-key",
    ...
    await expect(client.listModes({ cwd: "/tmp/cursor-sdk", force: true })).resolves.toEqual([
      expect.objectContaining({
        id: "yolo",
        label: "YOLO",
        isUnattended: true,
  ...
  test("listModels uses Cursor SDK discovery with provider API key and expands metadata", async () => {
    const runtime = new FakeCursorSdkRuntime();
    ...
    await expect(client.listModels({ cwd: "/workspace/project", force: false })).resolves.toEqual([
  ...
  test("empty SDK model discovery rejects before provider config can mask it", async () => {
    ...
    await expect(client.listModels({ cwd: "/workspace/project", force: false })).rejects.toThrow(
      /model discovery returned no models/u,
    );
  ...
  test("SDK operation failures rethrow redacted public errors", async () => {
    ...
    await expectRejectedMessageRedacted(() =>
      listFailureClient.listModels({ cwd: "/workspace/project", force: false }),
    );
  });
