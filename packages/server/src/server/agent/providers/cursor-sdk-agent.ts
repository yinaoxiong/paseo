...
export class CursorSdkAgentClient implements AgentClient {
  ...
  async fetchCatalog(_options: FetchCatalogOptions): Promise<ProviderCatalog> {
    const preflight = await this.preflight("fetchCatalog");
    return {
      models: expandCursorSdkModels(await this.listSdkRuntimeModels(preflight, "fetchCatalog")),
      modes: await this.fetchModeDefinitions(),
    };
  }
  ...
  private async fetchModeDefinitions(): Promise<AgentMode[]> {
